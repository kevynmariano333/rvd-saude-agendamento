  ...
  return (
    <PortalLayout user={auth.data} title="Administrar notas" subtitle="Correção auditável de status e exclusão definitiva de notas fiscais.">
      <section className="rounded-3xl border border-rvd-plum-soft bg-[#fafbfc] p-5 sm:p-7">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-start lg:justify-between">
          <div className="flex gap-4">
            ...
            <div>
              <h2 className="font-display text-2xl font-extrabold text-rvd-plum">Área exclusiva do Administrador</h2>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-rvd-plum">A exclusão é definitiva. Caso uma nota seja marcada como Recebida ou Concluída por engano, use <strong>Voltar para agendar</strong> para retornar o status a Agendado e permitir um novo horário. Todas as correções ficam registradas no histórico.</p>
            </div>
          </div>
          <div className="w-full lg:max-w-sm">
            ...
            <div className="relative mt-2"><Search className="absolute left-3 top-3 size-4 text-rvd-plum" /><Input id="admin-invoice-search" value={invoiceNumber} onChange={event => setInvoiceNumber(event.target.value)} placeholder="Número da NF" className="border-rvd-plum-soft bg-white pl-9 text-rvd-plum" /></div>
          </div>
        </div>
      </section>

      <section className="mt-6 overflow-hidden rounded-3xl border border-rvd-plum-soft bg-white">
        <div className="overflow-x-auto">
          <table className="w-full min-w-[980px] text-left">
            <thead className="bg-[#fafbfc]"><tr className="text-[11px] font-bold uppercase tracking-[0.1em] text-rvd-plum"><th className="px-6 py-5">Nota fiscal</th><th className="px-6 py-5">Fornecedor</th><th className="px-6 py-5">Status</th><th className="px-6 py-5">Registro</th><th className="px-6 py-5 text-right">Administração</th></tr></thead>
            <tbody>
      ...
      <Dialog open={Boolean(correctionTarget)} onOpenChange={open => !open && setCorrectionTarget(null)}>
        <DialogContent className="max-w-md rounded-[2rem] border-rvd-plum-soft bg-white p-7">
          <DialogHeader><div className="flex size-11 items-center justify-center rounded-2xl bg-rvd-blue-pale text-rvd-plum"><CalendarClock className="size-5" /></div><DialogTitle className="mt-4 font-display text-2xl font-extrabold text-rvd-plum">Retornar nota para Agendado?</DialogTitle><DialogDescription className="mt-2 text-rvd-plum">A nota <strong>{correctionTarget?.invoiceNumber || "sem número fiscal"}</strong> será corrigida de <strong>{correctionTarget ? statusLabel[correctionTarget.status] : ""}</strong> para <strong>Agendado</strong>. A confirmação de recebimento e a pré-nota serão removidas para que um novo horário seja definido. O histórico registrará esta correção administrativa.</DialogDescription></DialogHeader>
          <div className="mt-7 flex gap-3"><Button variant="ghost" onClick={() => setCorrectionTarget(null)} className="flex-1 border border-rvd-plum-soft bg-white font-bold text-rvd-plum hover:bg-rvd-plum-pale hover:text-rvd-plum">Cancelar</Button><Button onClick={() => correctionTarget && returnForRescheduling.mutate({ appointmentId: correctionTarget.id })} disabled={returnForRescheduling.isPending} className="flex-1 bg-rvd-plum font-bold text-white hover:bg-rvd-plum"><RotateCcw className="size-4" />{returnForRescheduling.isPending ? "Corrigindo..." : "Voltar para agendar"}</Button></div>
        </DialogContent>
      </Dialog>

      <Dialog open={Boolean(deleteTarget)} onOpenChange={open => !open && setDeleteTarget(null)}>
        <DialogContent className="max-w-md rounded-[2rem] border-rvd-plum-soft bg-white p-7">
          <DialogHeader><div className="flex size-11 items-center justify-center rounded-2xl bg-red-100 text-red-700"><FileWarning className="size-5" /></div><DialogTitle className="mt-4 font-display text-2xl font-extrabold text-rvd-plum">Excluir nota definitivamente?</DialogTitle><DialogDescription className="mt-2 text-rvd-plum">A nota <strong>{deleteTarget?.invoiceNumber || "sem número fiscal"}</strong> e todos os seus registros vinculados serão removidos. Esta ação não pode ser desfeita.</DialogDescription></DialogHeader>
          <div className="mt-7 flex gap-3"><Button variant="ghost" onClick={() => setDeleteTarget(null)} className="flex-1 border border-rvd-plum-soft bg-white font-bold text-rvd-plum hover:bg-rvd-plum-pale hover:text-rvd-plum">Cancelar</Button><Button onClick={() => deleteTarget && removeInvoice.mutate({ appointmentId: deleteTarget.id })} disabled={removeInvoice.isPending} className="flex-1 bg-red-700 font-bold text-white hover:bg-red-800"><Trash2 className="size-4" />{removeInvoice.isPending ? "Excluindo..." : "Excluir nota"}</Button></div>
        </DialogContent>
      </Dialog>
    </PortalLayout>
  );
