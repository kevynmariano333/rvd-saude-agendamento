...
function AppointmentRow({ item, activeStatus, canConfirm, onDetails, onHistory, onChat, onPreNote, onSchedule, onUpdate, onRescue, onFinalize }: { item: Appointment; activeStatus: StatusTab; canConfirm: boolean; onFinalize: () => void; onDetails: () => void; onHistory: () => void; onChat: () => void; onPreNote: () => void; onSchedule: () => void; onUpdate: (status: Exclude<PortalStatus, "pending">, reason?: string) => void; onRescue: () => void }) { const displaySupplier = item.invoiceSupplierName || item.supplierName || "Fornecedor"; const showReceivedMoment = item.status === "received" && Boolean(item.receivedAt); const hasConfirmedSchedule = hasConfirmedAppointmentMoment(item.status as PortalStatus); const displayedMoment = getAppointmentMomentForDisplay({ status: item.status as PortalStatus, scheduledFor: item.scheduledFor, receivedAt: item.receivedAt }); const destinatario = rotuloDoDestinatario(item.recipientCnpj); return <tr className="border-t border-line align-middle"><td className="px-3 py-4"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wide ${statusStyle[item.status as PortalStatus]}`}>{statusCopy[item.status as PortalStatus]}</span></td><td className="px-3 py-4"><p title={displaySupplier} className="line-clamp-2 max-w-40 font-bold leading-5 text-rvd-plum">{displaySupplier}</p><p title={item.supplierEmail ?? undefined} className="mt-1 max-w-40 truncate text-xs text-ink-soft">{item.supplierEmail}</p></td><td className="px-3 py-4"><p title={item.recipientCnpj ?? undefined} className="font-bold text-rvd-plum">{destinatario.titulo}</p><p className="mt-1 text-xs text-ink-soft">{destinatario.detalhe}</p></td><td className="px-3 py-4"><p className="font-display text-lg font-extrabold text-ink">{item.invoiceNumber || "—"}</p><p className="text-xs text-ink-soft">{sourceCopy[item.source]}</p></td><td className="px-3 py-4"><span className="rounded bg-rvd-plum-pale px-2 py-1 text-xs font-bold text-rvd-plum">{item.purchaseOrder || "—"}</span>{item.miroNumber && <p className="mt-1.5 text-[10px] font-bold uppercase tracking-wide text-ink-faint">MIRO {item.miroNumber}</p>}</td><td className="px-3 py-4">{activeStatus === "rejected" ? <p className="max-w-36 text-sm font-semibold leading-5 text-red-700">{item.rejectionReason || "Motivo não informado"}</p> : hasConfirmedSchedule ? <><p className="font-bold text-rvd-plum">{new Date(displayedMoment).toLocaleDateString("pt-BR")}</p><p className="text-sm text-ink-soft">{new Date(displayedMoment).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p>{showReceivedMoment && <p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-rvd-plum">Recebido em</p>}</> : <p className="max-w-32 text-sm font-semibold leading-5 text-rvd-plum">Aguardando confirmação</p>}</td><td className="px-3 py-4"><div className="flex items-center justify-end gap-2 text-rvd-plum"><ActionIcon label="Abrir detalhes da nota" icon={FileText} onClick={onDetails} /><ActionIcon label="Histórico de datas" icon={CalendarClock} onClick={onHistory} />{item.status === "scheduled" && <ActionIcon label={canConfirm ? "Reagendar" : "Sugerir outra data"} icon={CalendarDays} onClick={onSchedule} />}<ActionIcon label="Conversar sobre esta nota" icon={MessageSquare} onClick={onChat} /><ActionIcon label={item.preNoteConfirmedAt ? "Pré-nota confirmada" : "Confirmar pré-nota"} icon={ClipboardCheck} onClick={onPreNote} confirmed={Boolean(item.preNoteConfirmedAt)} /><PrimaryAction status={item.status as PortalStatus} canConfirm={canConfirm} onSchedule={onSchedule} onUpdate={onUpdate} onRescue={onRescue} onFinalize={onFinalize} /></div></td></tr>; }
