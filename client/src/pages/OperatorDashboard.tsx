...
const tabs: { id: StatusTab; label: string; icon: typeof Grid2X2 }[] = [
  { id: "all", label: "Todos", icon: Grid2X2 }, { id: "pending", label: "Pendente", icon: RotateCcw }, { id: "scheduled", label: "Agendado", icon: CalendarDays }, { id: "received", label: "Recebido", icon: ClipboardCheck }, { id: "completed", label: "Concluído", icon: CheckCircle2 }, { id: "rejected", label: "Rejeitado", icon: X },
];
...
export default function OperatorDashboard() {
  ...
  return <PortalLayout user={auth.data} title="Agendamentos" subtitle="Gestão e monitoramento de agendamentos e recebimentos." onLogout={() => logout.mutate()} actions={<div className="flex flex-wrap items-center gap-2 xl:flex-nowrap"><SegmentedControl value={dayShortcut} onChange={applyDayShortcut} options={[{ value: "todos", label: "Todos" }, { value: "hoje", label: "Hoje" }, { value: "amanha", label: "Amanhã" }]} className="shrink-0" />{podeConfirmar && <Button onClick={() => setReceiptOpen(true)} className="shrink-0 whitespace-nowrap rounded-xl bg-brand font-bold text-white hover:bg-brand/90"><ClipboardCheck className="size-4" />Recebimento sem agendamento</Button>}<Button onClick={() => setFiltersOpen(value => !value)} variant="outline" aria-pressed={filtersOpen} className="shrink-0 whitespace-nowrap rounded-xl border-line bg-surface font-bold text-ink-soft hover:bg-canvas hover:text-ink"><Filter className="size-4" />Filtros<ChevronDown className={`size-4 transition ${filtersOpen ? "rotate-180" : ""}`} /></Button></div>}>
    <section className="rounded-3xl bg-sunken p-4 sm:p-6">
      ...
      <section className="mt-6 overflow-hidden panel"><div className="overflow-x-auto"><table className="w-full min-w-[1150px] text-left"><thead className="bg-sunken"><tr className="text-[11px] font-bold uppercase tracking-[0.1em] text-ink-faint"><th className="px-6 py-5">Status</th><th className="px-6 py-5">Fornecedor</th><th className="px-6 py-5">Destinatário</th><th className="px-6 py-5">Nota fiscal</th><th className="px-6 py-5">Pedido</th><th className="px-6 py-5">{activeStatus === "rejected" ? "Motivo da recusa" : activeStatus === "pending" ? "Confirmação" : activeStatus === "received" ? "Recebimento" : "Agendamento"}</th><th className="px-6 py-5">Ícones de ação</th><th className="sticky right-0 z-10 shadow-[-10px_0_12px_-10px_rgba(0,0,0,0.18)] bg-sunken px-6 py-5 text-right">Ação</th></tr></thead><tbody>{agenda.isLoading ? <tr><td colSpan={8} className="py-20 text-center text-sm font-bold text-rvd-plum">Carregando agendamentos...</td></tr> : visibleAgenda.length ? visibleAgenda.map(item => <AppointmentRow key={item.id} item={item} activeStatus={activeStatus} canConfirm={podeConfirmar} onDetails={() => setDetails(item)} onHistory={() => setDateHistory(item)} onChat={() => setChatTarget(item)} onPreNote={() => item.preNoteConfirmedAt ? toast.info("Pré-nota já confirmada.") : setPreNoteTarget(item)} onSchedule={() => openSchedule(item)} onUpdate={(status, reason) => updateStatus.mutate({ appointmentId: item.id, status, rejectionReason: reason })} onRescue={() => rescue.mutate({ appointmentId: item.id })} />) : <tr><td colSpan={8} className="px-6 py-16 text-center"><Search className="mx-auto size-7 text-rvd-plum" /><p className="mt-3 font-bold text-rvd-plum">Nenhum agendamento encontrado</p><p className="mt-1 text-sm text-ink-soft">Ajuste os filtros ou selecione outra aba de status.</p></td></tr>}</tbody></table></div></section>
...
function AppointmentRow({ item, activeStatus, canConfirm, onDetails, onHistory, onChat, onPreNote, onSchedule, onUpdate, onRescue }: { item: Appointment; activeStatus: StatusTab; canConfirm: boolean; onDetails: () => void; onHistory: () => void; onChat: () => void; onPreNote: () => void; onSchedule: () => void; onUpdate: (status: Exclude<PortalStatus, "pending">, reason?: string) => void; onRescue: () => void }) { const displaySupplier = item.invoiceSupplierName || item.supplierName || "Fornecedor"; const showReceivedMoment = item.status === "received" && Boolean(item.receivedAt); const hasConfirmedSchedule = hasConfirmedAppointmentMoment(item.status as PortalStatus); const displayedMoment = getAppointmentMomentForDisplay({ status: item.status as PortalStatus, scheduledFor: item.scheduledFor, receivedAt: item.receivedAt }); return <tr className="border-t border-line align-middle"><td className="px-6 py-5"><span className={`inline-flex rounded-full px-3 py-1 text-xs font-bold uppercase tracking-wide ${statusStyle[item.status as PortalStatus]}`}>{statusCopy[item.status as PortalStatus]}</span></td><td className="px-6 py-5"><p className="max-w-40 font-bold leading-5 text-rvd-plum">{displaySupplier}</p><p className="mt-1 text-xs text-ink-soft">{item.supplierEmail}</p></td><td className="px-6 py-5"><p className="font-bold text-rvd-plum">{item.recipientCnpj || "—"}</p><p className="mt-1 text-xs text-ink-soft">Destinatário</p></td><td className="px-6 py-5"><p className="font-display text-lg font-extrabold text-ink">{item.invoiceNumber || "—"}</p><p className="text-xs text-ink-soft">{sourceCopy[item.source]}</p></td><td className="px-6 py-5"><span className="rounded bg-rvd-plum-pale px-2 py-1 text-xs font-bold text-rvd-plum">{item.purchaseOrder || "—"}</span></td><td className="px-6 py-5">{activeStatus === "rejected" ? <p className="max-w-36 text-sm font-semibold leading-5 text-red-700">{item.rejectionReason || "Motivo não informado"}</p> : hasConfirmedSchedule ? <><p className="font-bold text-rvd-plum">{new Date(displayedMoment).toLocaleDateString("pt-BR")}</p><p className="text-sm text-ink-soft">{new Date(displayedMoment).toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</p>{showReceivedMoment && <p className="mt-1 text-[10px] font-bold uppercase tracking-wide text-rvd-plum">Recebido em</p>}</> : <p className="max-w-32 text-sm font-semibold leading-5 text-rvd-plum">Aguardando confirmação</p>}</td><td className="px-6 py-5"><div className="flex items-center gap-3 text-rvd-plum"><ActionIcon label="Abrir detalhes da nota" icon={FileText} onClick={onDetails} /><ActionIcon label="Histórico de datas" icon={CalendarClock} onClick={onHistory} />{item.status === "scheduled" && <ActionIcon label={canConfirm ? "Reagendar" : "Sugerir outra data"} icon={CalendarDays} onClick={onSchedule} />}<ActionIcon label="Conversar sobre esta nota" icon={MessageSquare} onClick={onChat} /><ActionIcon label={item.preNoteConfirmedAt ? "Pré-nota confirmada" : "Confirmar pré-nota"} icon={ClipboardCheck} onClick={onPreNote} confirmed={Boolean(item.preNoteConfirmedAt)} /></div></td><td className="sticky right-0 z-10 shadow-[-10px_0_12px_-10px_rgba(0,0,0,0.18)] bg-surface px-6 py-5 text-right"><PrimaryAction status={item.status as PortalStatus} canConfirm={canConfirm} onSchedule={onSchedule} onUpdate={onUpdate} onRescue={onRescue} /></td></tr>; }
...
function PrimaryAction({ status, canConfirm, onSchedule, onUpdate, onRescue }: { status: PortalStatus; canConfirm: boolean; onSchedule: () => void; onUpdate: (status: Exclude<PortalStatus, "pending">, reason?: string) => void; onRescue: () => void }) { const buttonClass = "h-9 rounded-xl px-4 text-xs font-bold"; if (status === "pending") return <Button onClick={onSchedule} className={`${buttonClass} bg-brand text-white hover:bg-brand`}>{canConfirm ? "Agendar" : "Sugerir data"}</Button>; if (status === "scheduled") return <div className="flex justify-end gap-2"><Button onClick={onSchedule} variant="ghost" className={`${buttonClass} border border-line text-rvd-plum hover:bg-rvd-plum-pale hover:text-rvd-plum`}>{canConfirm ? "Reagendar" : "Sugerir data"}</Button><Button onClick={() => onUpdate("received")} className={`${buttonClass} bg-brand text-white hover:bg-brand`}>Recebido</Button><Button onClick={() => onUpdate("rejected", canConfirm ? "Recusado pelo operador" : "Recusado pelo planejador")} variant="ghost" className={`${buttonClass} border border-line text-rvd-plum hover:bg-rvd-plum-pale hover:text-rvd-plum`}>Rejeitar</Button></div>; if (status === "received") return <Button onClick={() => onUpdate("completed")} className={`${buttonClass} bg-brand text-white hover:bg-brand`}>Concluir</Button>; if (status === "rejected") return <Button onClick={onRescue} className={`${buttonClass} bg-rvd-blue text-rvd-plum hover:bg-rvd-blue-pale`}><RefreshCw className="size-3.5" />Resgatar</Button>; return <span className="text-xs font-bold text-rvd-plum">Consulta</span>; }
