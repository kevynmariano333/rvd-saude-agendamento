...
export default function OperatorDashboard() {
  ...
  }), [activeStatus, dailyFilterDate, date, dateEnd, invoiceNumber, itemCount, itemCountOperator, onlyUrgent, pagina, purchaseOrder, recipientCnpj, sapCode, somenteServico, supplierCnpj, supplierName, tomorrowFilterDate]);
  // Trocar de aba ou de filtro recomeça a contagem: a página 3 de uma busca não
  // é a página 3 da seguinte.
  useEffect(() => { setPagina(1); }, [activeStatus, dailyFilterDate, date, dateEnd, invoiceNumber, itemCount, itemCountOperator, onlyUrgent, purchaseOrder, recipientCnpj, sapCode, supplierCnpj, supplierName, tomorrowFilterDate]);
  ...
  const clearFilters = () => { setDate(""); setDateEnd(""); setDailyFilterDate(null); setTomorrowFilterDate(null); setInvoiceNumber(""); setSupplierName(""); setRecipientCnpj(""); setPurchaseOrder(""); setSapCode(""); setSupplierCnpj(""); setItemCountOperator(">="); setItemCount(""); setOnlyUrgent(false); setActiveStatus("all"); };
  ...
  return <PortalLayout user={auth.data} title="Agendamentos" subtitle="Gestão e monitoramento de agendamentos e recebimentos." onLogout={() => logout.mutate()} actions={podeConfirmar ? <div className="flex flex-wrap items-center gap-2"><Button onClick={() => setNotaDeServicoAberta(true)} variant="outline" className="shrink-0 whitespace-nowrap rounded-xl border-line bg-surface font-bold text-rvd-plum hover:bg-rvd-plum-pale"><Wrench className="size-4" />Nota de serviço</Button><Button onClick={() => setReceiptOpen(true)} className="shrink-0 whitespace-nowrap rounded-xl bg-brand font-bold text-white hover:bg-brand/90"><ClipboardCheck className="size-4" />Recebimento sem agendamento</Button></div> : undefined}>
    ...
    <ConfirmacaoDialog
      ...
      corpo="A nota volta para o começo da fila. O recebimento e o número MIRO são desfeitos, e o que ela tinha fica registrado no histórico."
