...
export default function OperatorDashboard() {
  ...
  // Quantas mensagens novas em cada nota. O sino do topo diz que existe
  // mensagem; sem isto, o operador teria que abrir conversa por conversa para
  // descobrir em qual.
  const naoLidas = trpc.messages.naoLidasPorNota.useQuery(undefined, { refetchInterval: 15_000 });
  const naoLidasPorNota = useMemo(() => new Map((naoLidas.data ?? []).map(linha => [linha.appointmentId, linha.quantas])), [naoLidas.data]);
 ...
 */
function ActionIcon({ label, icon: Icon, onClick, confirmed = false, somenteLeitura = false, contagem = 0 }: { label: string; icon: typeof FileText; onClick: () => void; confirmed?: boolean; somenteLeitura?: boolean; contagem?: number }) {
  ...
  return (
    <button onClick={onClick} title={label} className={`relative rounded-lg p-1.5 hover:bg-rvd-plum-pale ${confirmed ? `${cor} hover:bg-emerald-100` : ""}`}>
      ...
      {contagem > 0 && (
        <span className="absolute -right-0.5 -top-0.5 flex size-4 items-center justify-center rounded-full bg-state-stop px-1 text-[9px] font-bold leading-none text-white">
