...
export default function OperatorDashboard() {
  ...
  return <PortalLayout user={auth.data} title="Agendamentos" subtitle="Gestão e monitoramento de agendamentos e recebimentos." onLogout={() => logout.mutate()} actions={podeConfirmar ? <div className="flex flex-wrap items-center gap-2"><Button onClick={() => setNotaDeServicoAberta(true)} variant="outline" className="shrink-0 whitespace-nowrap rounded-xl border-line bg-surface font-bold text-rvd-plum hover:bg-rvd-plum-pale"><Wrench className="size-4" />Nota de serviço</Button><Button onClick={() => setReceiptOpen(true)} className="shrink-0 whitespace-nowrap rounded-xl bg-brand font-bold text-white hover:bg-brand/90"><ClipboardCheck className="size-4" />Recebimento sem agendamento</Button></div> : undefined}>
    <section className="rounded-3xl bg-sunken p-4 sm:p-6">
      ...
      {filtersOpen && <div className="mt-5 panel p-5"><div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4"><FilterField label="Número da nota" value={invoiceNumber} onChange={setInvoiceNumber} placeholder="NF-e..." /><FilterField label="Nome do fornecedor" value={supplierName} onChange={setSupplierName} placeholder="Parte do nome..." /><SeletorDeDestinatario value={recipientCnpj} onChange={setRecipientCnpj} /><div><Label className="text-xs font-bold uppercase tracking-wide text-rvd-plum">Data inicial</Label><Input type="date" value={date} onChange={event => { setDate(event.target.value); setDailyFilterDate(null); setTomorrowFilterDate(null); }} className="mt-2 border-line text-rvd-plum" /></div></div><Button onClick={clearFilters} variant="ghost" className="mt-5 px-0 text-rvd-plum hover:bg-transparent hover:text-rvd-plum"><X className="size-4" />Limpar filtros</Button></div>}
      <div className="mt-5 flex flex-wrap items-center gap-2 border-t border-line pt-5">{tabs.map(tab => { const Icon = tab.icon; const count = counters[tab.id]; const active = activeStatus === tab.id; const showsCounter = CONTADOR_NA_ABA.has(tab.id);
