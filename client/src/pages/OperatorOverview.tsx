    ...
    <PortalLayout user={auth.data} title="Dashboard" subtitle="Acompanhe indicadores, recebimentos e prioridades da operação." onLogout={() => logout.mutate()}>
      <div className="space-y-6">
        ...
        <section className="grid gap-4 lg:grid-cols-3">
          <MetricCard label="Pendentes de Agendamento" value={data?.pendingCount ?? 0} amountCents={data?.pendingTotalCents ?? 0} detail="Aguardam confirmação" icon={Clock3} iconClass="bg-amber-50 text-amber-500" />
          <MetricCard label="Agendadas" value={data?.scheduledCount ?? 0} amountCents={data?.scheduledTotalCents ?? 0} detail={`Confirmadas em ${monthName}`} icon={CalendarDays} iconClass="bg-rvd-lilac-blue text-rvd-plum" />
          <MetricCard label="Recebidas" value={data?.receivedCount ?? 0} amountCents={data?.receivedTotalCents ?? 0} detail={`Notas recebidas em ${monthName}`} icon={PackageCheck} iconClass="bg-emerald-50 text-emerald-600" />
        </section>
          ...
          <div className="mt-5 h-72">
            {data?.dailyReceived.some(item => item.total > 0) ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={data.dailyReceived} margin={{ top: 22, right: 4, left: -22, bottom: 0 }}>
                  ...
                  <YAxis allowDecimals={false} tickLine={false} axisLine={false} tick={{ fill: "#a28aa4", fontSize: 11 }} />
                  <Tooltip cursor={{ fill: "#f8f0f8" }} contentStyle={{ borderRadius: 14, borderColor: "#e9d4ea" }} formatter={(value: number) => [value, "Notas recebidas"]} />
                  <Bar dataKey="total" fill="#39c99b" radius={[6, 6, 2, 2]} maxBarSize={30}>
