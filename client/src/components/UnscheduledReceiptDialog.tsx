...
export default function UnscheduledReceiptDialog({ open, onOpenChange, onRegistered }: { open: boolean; onOpenChange: (open: boolean) => void; onRegistered: () => void }) {
  ...
  return <Dialog open={open} onOpenChange={onOpenChange}><DialogContent style={{ backgroundColor: "#ffffff", opacity: 1 }} className="max-w-2xl overflow-hidden rounded-[2rem] !border !border-rvd-plum-soft !bg-white p-0 opacity-100 shadow-2xl"><DialogHeader className="border-b border-rvd-plum-soft bg-white px-7 py-6"><DialogTitle className="font-display text-2xl font-extrabold text-rvd-plum">Recebimento sem agendamento</DialogTitle><DialogDescription className="text-rvd-plum">Registre uma nota que chegou sem agendamento prévio.</DialogDescription></DialogHeader><div className="bg-white p-7"><button type="button" onClick={() => inputRef.current?.click()} className="flex min-h-44 w-full flex-col items-center justify-center rounded-3xl border-2 border-dashed border-rvd-plum-soft bg-[#f5ecf5] p-6 text-center shadow-inner transition hover:bg-rvd-plum-pale"><span className="rounded-2xl bg-white p-3 text-rvd-plum shadow-sm"><Upload className="size-6" /></span><p className="mt-4 font-bold text-rvd-plum">{file ? file.name : "Clique para carregar o XML da nota"}</p><p className="mt-1 text-sm text-rvd-plum">O XML preenche todos os campos automaticamente.</p><input ref={inputRef} type="file" accept=".xml,application/xml,text/xml" className="hidden" onChange={event => setFile(event.target.files?.[0] ?? null)} /></button><div className="mt-4 flex items-center gap-2 rounded-2xl border border-rvd-plum-soft bg-white px-4 py-3 text-sm font-semibold text-rvd-plum"><FileWarning className="size-4" />Não possui o XML da nota? Solicite-o ao fornecedor antes de registrar o recebimento.</div><div className="mt-7 flex justify-end gap-3 bg-white"><Button variant="ghost" onClick={() => onOpenChange(false)} className="font-bold text-rvd-plum hover:bg-rvd-plum-pale hover:text-rvd-plum">Cancelar</Button><Button onClick={register} disabled={!file || receipt.isPending} className="h-12 rounded-2xl bg-rvd-plum px-6 font-bold text-white hover:bg-rvd-plum">{receipt.isPending ? "Registrando..." : "Registrar recebimento"}</Button></div></div></DialogContent></Dialog>;
}
