...
export type AppointmentDetail = { id: number; supplierId: number; supplierName: string | null; supplierEmail: string | null; serviceType: string; scheduledFor: Date; notes: string | null; source: PortalSource; preNoteConfirmedAt: Date | null; preNoteConfirmedBy: number | null; urgenteMarcadoEm: Date | null; urgenteMotivo: string | null; xmlUrl: string | null; xmlFileName: string | null; invoiceNumber: string | null; invoiceAccessKey: string | null; purchaseOrder: string | null; invoiceSupplierName: string | null; invoiceSupplierCnpj: string | null; supplierCnpj: string | null; recipientCnpj: string | null; invoiceIssuedAt: Date | null; invoiceTotalCents: number | null; invoiceVolumeCount: number | null; receivedAt: Date | null; miroNumber: string | null; quotationNumber: string | null; memorizedOrder: string | null; hisEntryDocument: string | null; hisExitDocument: string | null; backlogReasonCode: string | null; backlogReason: string | null; treatedAt: Date | null; rejectionReason: string | null; status: PortalStatus; createdAt: Date; updatedAt: Date };
 ...
 *
 * A urgência que vem do número do pedido o sistema deduz sozinho e ninguém
 * mexe. Esta é a outra: a entrega que o ERP não sabe que virou urgente porque o
 * estoque acabou ou a cirurgia foi antecipada — e quem sabe disso é quem
 * planeja a semana.
 *
 * Pede o motivo ao marcar, e não ao tirar: prioridade sem motivo escrito vira
 * uma marca que ninguém sabe se ainda vale.
 */
function BotaoDeUrgencia({ appointmentId, marcada }: { appointmentId: number; marcada: boolean }) {
  const utils = trpc.useUtils();
  const [perguntando, setPerguntando] = useState(false);
  const [motivo, setMotivo] = useState("");
  const marcar = trpc.appointments.marcarUrgencia.useMutation({
    onSuccess: () => {
      setPerguntando(false);
      setMotivo("");
      void utils.appointments.list.invalidate();
      void utils.appointments.byId.invalidate();
  ...
  if (marcada) {
    return (
      <Button
        type="button"
        variant="ghost"
        onClick={() => marcar.mutate({ appointmentId, urgente: false })}
        disabled={marcar.isPending}
        className="h-9 rounded-xl border border-state-stop bg-state-stop-bg text-[13px] font-bold text-state-stop hover:bg-state-stop-bg hover:text-state-stop"
      >
        <AlertTriangle className="size-4" />
        {marcar.isPending ? "Tirando..." : "Tirar a urgência"}
      </Button>
    );
  }
  ...
  if (!perguntando) {
    return (
      <Button type="button" variant="ghost" onClick={() => setPerguntando(true)} className="h-9 rounded-xl border border-line bg-surface text-[13px] font-bold text-rvd-plum hover:bg-rvd-plum-pale hover:text-rvd-plum">
        <AlertTriangle className="size-4" />
        Marcar como urgente
      </Button>
    );
  ...
  return (
    <div className="flex items-center gap-2">
      <Input
        ...
        placeholder="Por que não pode esperar?"
        maxLength={255}
        className="h-9 w-56 border-line bg-surface text-[13px]"
        onKeyDown={evento => {
          if (evento.key === "Enter" && motivo.trim()) marcar.mutate({ appointmentId, urgente: true, motivo: motivo.trim() });
          if (evento.key === "Escape") setPerguntando(false);
        }}
      />
      <Button
        type="button"
        onClick={() => marcar.mutate({ appointmentId, urgente: true, motivo: motivo.trim() })}
        disabled={!motivo.trim() || marcar.isPending}
        className="h-9 rounded-xl bg-state-stop px-3 text-[13px] font-bold text-white hover:bg-state-stop"
      >
        {marcar.isPending ? "Marcando..." : "Marcar"}
      </Button>
      <Button type="button" variant="ghost" onClick={() => setPerguntando(false)} className="h-9 px-2 text-[13px] font-bold text-rvd-plum hover:bg-rvd-plum-pale hover:text-rvd-plum">
        Cancelar
      </Button>
    </div>
  );
...
export default function AppointmentDetailsDialog({ appointment, open, onOpenChange, onHistory }: { appointment: AppointmentDetail | null; open: boolean; onOpenChange: (open: boolean) => void; onHistory: () => void }) {
  ...
  // A urgência sai da consulta da nota, e não da linha que abriu a janela: a
  // linha foi carregada antes e não sabe do que acabou de ser marcado aqui
  // dentro. Enquanto a consulta não chega, vale o que a linha trouxe.
  const urgencia = notaCompleta.data
    ? { marcadaEm: notaCompleta.data.urgenteMarcadoEm, motivo: notaCompleta.data.urgenteMotivo }
    : { marcadaEm: appointment.urgenteMarcadoEm, motivo: appointment.urgenteMotivo };
