...
import { AlertTriangle, Banknote, CalendarDays, CheckCircle2, ClipboardList, FileText, History, Package, ShoppingCart, Users } from "lucide-react";
...
import { casarItensComPedido } from "@shared/casamentoDePedido";
...
export default function AppointmentDetailsDialog({ appointment, open, onOpenChange, onHistory }: { appointment: AppointmentDetail | null; open: boolean; onOpenChange: (open: boolean) => void; onHistory: () => void }) {
  ...
  const doPedido = trpc.appointments.pedidosDaNota.useQuery({ appointmentId: appointment?.id ?? 0 }, { enabled: open && Boolean(appointment), retry: false });
